...
import { EYE_SHAPES, MODULE_SHAPES, geometry, matrix, svg, type EyeShape, type ModuleShape } from '../lib/render';
...
export default function StylePanel({ ui, options, onChange }: Props) {
  ...
  /** Live thumbnails, drawn with the user's own colours. */
        ...
        <div class="grid grid-cols-3 gap-2 sm:grid-cols-6">
          {thumbs.map(({ preset, svg: markup }) => (
            <label
              key={preset.id}
              class="thumb"
              data-active={active?.id === preset.id}
            >
              <input
                type="radio"
                name="qr-style"
                ...
                onChange={() => set({ modules: preset.modules, eyes: preset.eyes })}
              />
